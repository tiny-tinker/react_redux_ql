  ...
  (function ($) {
    ...
    function checkMenu() {
      if ($(window).scrollTop() > offset && !navigationContainer.hasClass('is-fixed')) {
        navigationContainer.addClass('is-fixed').find('.st-nav-trigger').one('webkitAnimationEnd oanimationend msAnimationEnd animationend', () => {
    ...
    $.fn.waterwheelCarousel = function (startingOptions) {
      ...
      function preCalculatePositionProperties() {
        ...
        const $firstItem = data.itemsContainer.find('img:first');
      ...
      function performCalculations($item, newPosition) {
        ...
        const newWidth = distanceFactor * $item.data('original_width');
        const newHeight = distanceFactor * $item.data('original_height');
        const widthDifference = Math.abs($item.width() - newWidth);
        const heightDifference = Math.abs($item.height() - newHeight);
      ...
      function rotateCarousel(rotations) {
        ...
        if (data.currentlyMoving === false) {
          ...
          for (let i = 0; i < data.totalItems; i++) {
            ...
            const currentPosition = $item.data('currentPosition');
      ...
      $(this).find('img').bind('click', function () {
        ...
        const rotations = Math.abs(itemPosition);
        if (itemPosition == 0) {
      ...
      $(this).find('a').bind('click', function (event) {
        const isCenter = $(this).find('img').data('currentPosition') == 0;
...
$(document).ready(() => {
  ...
  $('#button_right').click(() => {
    if (roadmap_value < 7) {
      roadmap_value += 1;
      const nameOfclass = `rotated${roadmap_value}`;
      const rotatedDiv = $(`.${nameOfclass}`);
      rotatedDiv.trigger('click');
  ...
  $('#button_left').click(() => {
    if (roadmap_value > 1) {
      ...
      const rotatedDiv = $(`.${nameOfclass}`);
      rotatedDiv.trigger('click');
      $('#button_right').css({ opacity: 1 });
    }
    if (roadmap_value == 1) {
      console.log(roadmap_value);
      // noinspection JSAnnotator
      $('#button_left').css({ opacity: 0.1 });
    }
  });
